...
import com.manteng.common.SignUtil;
...
//    public final static HttpCache sHttpCache = new HttpCache();
    ...
    public HttpUtils(int connTimeout, String userAgent) {
        ...
        schemeRegistry.register(new Scheme("http", PlainSocketFactory.getSocketFactory(), 9999));
        schemeRegistry.register(new Scheme("https", DefaultSSLSocketFactory.getSocketFactory(), 443));
        schemeRegistry.register(new Scheme("https", DefaultSSLSocketFactory.getSocketFactory(), 9998));
...
//    }
    ...
    private List<Header> genHeaders(RequestParams params){
		List<NameValuePair> paramMap = params.getQueryStringParams();
		ArrayList<Header> headers = new ArrayList<Header>();
		headers.add(new BasicHeader(SN, String.valueOf(VERSION_CODE)));
		headers.add(new BasicHeader(IMEI, DEVICE_IMEI));
		headers.add(new BasicHeader(ID, USERID));
		headers.add(new BasicHeader(TOKEN, SignUtil.makeSignForString(
				getSignSecret(), paramMap)));
		headers.add(new BasicHeader(OS_TYPE, String.valueOf(1)));
		
		return headers;
	}
    
    private String getSignSecret() {
		return DEVICE_IMEI + VERSION_NAME;
	}
...
//        handler.setExpiry(currentRequestExpiry);
        handler.setHttpRedirectHandler(httpRedirectHandler);
        params.addHeaders(genHeaders(params));
        request.setRequestParams(params, handler);
        if (params != null) {
            handler.setPriority(params.getPriority());
        }
